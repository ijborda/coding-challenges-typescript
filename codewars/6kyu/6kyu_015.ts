...
export function tribonacci (sig: number[], n: number): number[] {
  const SUM_LAST = sig.length;
  for (let i = SUM_LAST; i < n; i++) {
    sig.push(sig.slice(-SUM_LAST).reduce((acc, a) => acc + a, 0));
  }
