...
export const mean = (town:string, s:string): number => {
  const townData = extractData(town, s);
  if (!townData.length) {
    return -1;
  }
  return townData.reduce((acc, a) => acc + a, 0) / townData.length;
};
...
export const variance = (town:string, s:string): number => {
  ...
  const townMean = mean(town, s);
  return townData.reduce((acc, a) => acc + Math.pow(townMean - a, 2), 0) / townData.length;
};

const extractData = (town: string, s: string): number[] => {
  const townRecord = s
    .split('\n')
    .filter(record => record.split(':')[0] === town)[0];
  if (!townRecord) {
    return [];
  }
  const townData = townRecord
    .split(':')[1]
    .split(',')
