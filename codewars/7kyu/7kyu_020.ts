...
export function rowSumOddNumbers (n: number): number {
  // const nthStart = Array.from({ length: n }, (_, i) => i + 1).reduce((acc, a) => acc + a, 0) - n + 1;
  // const start = 2 * nthStart - 1;
  // const nthRow = Array.from({ length: n }, (_, i) => start + 2 * i);
  // return nthRow.reduce((acc, a) => acc + a, 0);
